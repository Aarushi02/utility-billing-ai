...
def run_tariff_pipeline(pdf_path: Path, raw_bill_document_id: int = None):
    ...
    try:
        ...
        raise RuntimeError(
            "Tariff extraction timed out after 5 minutes during Step 1"
        )
    ...
    if result.returncode != 0:
        ...
        print(f"STDERR:\n{result.stderr}")
        raise RuntimeError(
            f"pagewise_text_extractor.py failed: {result.stderr}"
        )
    ...
    if not file_exists_in_s3(s3_key_raw):
        raise RuntimeError(
            f"raw_extracted_tarif.json was not created in S3: {s3_key_raw}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Tariff extraction timed out after 5 minutes during Step 2"
        )
    ...
    if result.returncode != 0:
        ...
        print(f"STDERR:\n{result.stderr}")
        raise RuntimeError(
            f"group_extracted_raw_text.py failed: {result.stderr}"
        )
    ...
    if not file_exists_in_s3(s3_key_grouped):
        ...
        print(f"STDERR:\n{result.stderr}")
        raise RuntimeError(
            f"grouped_tariffs.json was not created in S3: {s3_key_grouped}"
        )

    print("Step 2/3: Tariff grouping completed.")

    # ======================================================
    # 3) extract_logic_llm_call.py
    # ======================================================
    ...
    try:
        ...
        raise RuntimeError(
            "Tariff extraction timed out after 5 minutes during Step 3"
        )
    ...
    if result.returncode != 0:
        ...
        print(f"STDERR:\n{result.stderr}")
        raise RuntimeError(
            f"extract_logic_llm_call.py failed: {result.stderr}"
        )
    ...
    if not file_exists_in_s3(s3_key_logic):
        raise RuntimeError(
            f"final_logic_output.json was not created in S3: {s3_key_logic}"
        )
    ...
    print("=" * 60)
    print(
        f"Grouped Tariffs: s3://{get_s3_key('processed', 'grouped_tariffs.json')}"
    )
    print(
        f"Final Logic Output: s3://{get_s3_key('processed', 'final_logic_output.json')}"
    )
    print("=" * 60 + "\n")

    return {
        "grouped_tariffs": get_s3_key(
            "processed",
            "grouped_tariffs.json",
        ),
        "final_logic": get_s3_key(
            "processed",
